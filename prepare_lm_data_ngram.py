...
def main():
    ...
    if args.do_data:
        data_path = config['data_dir'] / "corpus/train/shard"
        files = sorted([f for f in data_path.iterdir() if f.exists() and '.txt' in str(f)])

        for idx in range(2, args.file_num):
            ...
            with save_filename.open('w') as fw:
                for file_idx in range(len(files)):
                    print('file idx: ', file_idx, '/', str(len(files)))
