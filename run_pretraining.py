...
class PregeneratedDataset(Dataset):
    def __init__(self, training_path, file_id, tokenizer, data_name, reduce_memory=False):
        ...
        np.save(config['prepre_load_data'] + '/input_ids_' + f"{data_name}_file_{self.file_id}", input_ids)
        np.save(config['prepre_load_data'] + '/lm_label_ids_' + f"{data_name}_file_{self.file_id}", lm_label_ids)
        np.save(config['prepre_load_data'] + '/num_samples_' + f"{data_name}_file_{self.file_id}", num_samples)
        np.save(config['prepre_load_data'] + '/seq_len_' + f"{data_name}_file_{self.file_id}", seq_len)
        np.save(config['prepre_load_data'] + '/input_masks_' + f"{data_name}_file_{self.file_id}", input_masks)
        np.save(config['prepre_load_data'] + '/segment_ids_' + f"{data_name}_file_{self.file_id}", segment_ids)
        np.save(config['prepre_load_data'] + '/is_nexts_' + f"{data_name}_file_{self.file_id}", is_nexts)

        # num_samples = np.load('/data1/nghind/workspaces/data/huggingface-pytorch-transformers/train_bert_from_scratch/dataset/output_pretrained_news_128/epoch_' + str(epoch) + '_num_samples.npy')
        # seq_len = np.load('/data1/nghind/workspaces/data/huggingface-pytorch-transformers/train_bert_from_scratch/dataset/output_pretrained_news_128/epoch_' + str(epoch) + '_seq_len.npy')
        # segment_ids = np.load('/data1/nghind/workspaces/data/huggingface-pytorch-transformers/train_bert_from_scratch/dataset/output_pretrained_news_128/epoch_' + str(epoch) + '_segment_ids.npy')
        # lm_label_ids = np.load('/data1/nghind/workspaces/data/huggingface-pytorch-transformers/train_bert_from_scratch/dataset/output_pretrained_news_128/epoch_' + str(epoch) + '_lm_label_ids.npy', mmap_mode='r')
        # input_ids = np.load('/data1/nghind/workspaces/data/huggingface-pytorch-transformers/train_bert_from_scratch/dataset/output_pretrained_news_128/epoch_' + str(epoch) + '_input_ids.npy', mmap_mode='r')
        # input_masks = np.load('/data1/nghind/workspaces/data/huggingface-pytorch-transformers/train_bert_from_scratch/dataset/output_pretrained_news_128/epoch_' + str(epoch) + '_input_masks.npy', mmap_mode='r')
        # is_nexts = np.load('/data1/nghind/workspaces/data/huggingface-pytorch-transformers/train_bert_from_scratch/dataset/output_pretrained_news_128/epoch_' + str(epoch) + '_is_nexts.npy', mmap_mode='r')
...
def main():
    ...
    if args.local_rank == -1 or args.no_cuda:
        device = torch.device(f"cuda:4" if torch.cuda.is_available() and not args.no_cuda else "cpu")
    ...
    if args.n_gpu > 1:
        model = torch.nn.DataParallel(model, device_ids=[4, 5])
